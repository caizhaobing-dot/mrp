<template>
  <div>
    ...
    >
      ...
      <template #toolbar>
        <a-button v-if="$auth('/api/PartMaterial/Add')" type="primary" @click="openModal({})">
          {{ t('common.add') }}
        </a-button>
        ...
        <a-button
          v-if="$auth('/api/PartMaterial/Delete')"
          type="error"
...
<script lang="tsx" setup>
  ...
  import action from '@/api/base/partMaterial'
  ...
  const {
    ...
    formDataCallback: data => {
      return { groupType: data.partGroup.type }
    },
  })
